...
JNIEXPORT jint JNICALL Java_com_chinamobile_smartgateway_commservices_UsbServiceImp_usbWriteJni
(JNIEnv *env, jobject obj, jint handle, jbyteArray buf, jint offset, jint count )
{
	int ret = 0;
	jint response = 0;
	jbyte* bufByte = NULL;
	...
	ret = joyme_cmcc_usbWrite(&response, handle, bufByte, offset, count);
